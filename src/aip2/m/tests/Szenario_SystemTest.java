...
public class Szenario_SystemTest {
	...
	public void setUp() throws Exception {
		...
		kunden.add(hes.getIKundenModulExtern().erstelleKunde("Meier, Peter", "Bla 12 Dorf"));
		kunden.add(hes.getIKundenModulExtern().erstelleKunde("Meier, Bert", "Hust 13 Dorf"));
		
		produkte = new HashSet<ProduktTyp>();
		produkte.add(hes.getIProduktModulExtern().erstelleProdukt("SSD 1337", 10000));
		produkte.add(hes.getIProduktModulExtern().erstelleProdukt("SSD ABC", 5000));
	}
	...
	public void test() {
		...
		List<KundenTyp> kundenL = hes.getIKundenModulExtern().sucheKunden("Meier");
		Set<KundenTyp> kundenS = new HashSet<KundenTyp>(kundenL);
		assertTrue(kunden.equals(kundenS));
		
		// Call Agent sucht das Produkt
		List<ProduktTyp> produktL = hes.getIProduktModulExtern().sucheProdukt("SSD");
		...
		AngebotTyp angebot = hes.getIAngebotAuftragModulExtern().erstelleAngebot(kundenL.get(0), 
				new Date(), pMap, 1000000);
		...
		List<AngebotTyp> angebotsListe = hes.getIAngebotAuftragModulExtern().sucheAngebote(kundenL.get(0));
		assertTrue(angebote.equals(angebotsListe));
		
		// Call Agent erstellt Auftrag
		AuftragTyp auftrag = hes.getIAngebotAuftragModulExtern().erstelleAuftrag(angebot);
